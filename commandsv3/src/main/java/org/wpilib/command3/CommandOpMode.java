 ...
 *
 * <p>This class provides a periodic execution loop that drives a {@link Scheduler} and integrates
 * with the Driver Station lifecycle (selected, enabled/disabled). It offers a few convenience
 * triggers representing the state of the opmode and exposes lifecycle hooks required by the {@link
 * OpMode} interface.
 *
 * <p>Key behaviors and guarantees:
 *
 * <ul>
 *   <li>The scheduler is run while the opmode is active and enabled.
 *   <li>Watchdog epochs are tracked to detect loop overruns and produce helpful diagnostics when
 *       the loop exceeds the configured period.
 *   <li>NetworkTables are flushed after each loop so telemetry is delivered promptly to the driver
 *       station.
 * </ul>
 *
 * <p>Note: This class is specific to the commandsv3 scheduler integration and intentionally
 * documents its own behavior (it does not inherit or re-use docs from other opmode base classes).
 */
public class CommandOpMode implements OpMode {
  ...
  /**
   * Trigger that is active while this opmode object is currently selected on the driver station.
   * Useful for wiring commands or actions that should run only when this opmode is the chosen
   * program.
   */
  public final Trigger selected;

  /**
   * Trigger that is active while the Driver Station reports the robot is disabled and this opmode
   * is selected. This is the logical "disabled" condition for building command bindings.
   */
  public final Trigger disabled;

  /**
   * Trigger that is active while the Driver Station reports the robot is enabled and this opmode is
   * selected. This is the logical "enabled" condition for building command bindings.
   */
   ...
   * Constructor.
   *
   * @param scheduler the Scheduler instance this opmode should drive. Typically you will pass
   *     {@link Scheduler#getDefault()}.
   */
  ...
  /**
   * Called periodically while the opmode is selected on the Driver Station and the Driver Station
   * reports the robot is disabled. Implementations may override this to add behavior that should
   * run while disabled (e.g. input polling or diagnostics). The default implementation runs the
   * scheduler.
   */
  @Override
  ...
  /**
   * Called when the opmode is stopped; destroys the native notifier. This will be called by the
   * runtime when the opmode is no longer needed.
   */
  @Override
  ...
  /**
   * Called when the opmode object is being closed. If the notifier still exists it is destroyed.
  ...
  /**
   * Prints the list of watchdog epochs that have been recorded for the most recent loop. This can
   * be useful for debugging timing issues.
   */
