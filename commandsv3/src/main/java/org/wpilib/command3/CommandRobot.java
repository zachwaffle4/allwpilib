 ...
 *
 * <p>This class wires a {@link Scheduler} into the robot lifecycle: it registers a periodic
 * callback that invokes {@link #robotPeriodic()} on the provided scheduler. It also provides
 * convenient factory methods for creating and registering {@link CommandOpMode} instances for the
 * common robot modes (autonomous, teleoperated, and test).
 *
 * <p>Typical usage:
 *
 * <pre>
...
public abstract class CommandRobot extends OpModeRobot {
  /**
   * Create a CommandRobot that registers the provided scheduler's periodic callback which will call
   * {@link #robotPeriodic()} every scheduler cycle.
   *
  ...
  /**
   * Periodic hook invoked by the scheduler each cycle. Subclasses should override this method to
   * perform robot-wide periodic work (telemetry, diagnostics, subsystem updates, etc.). This method
   * is called from the scheduler's periodic context.
   */
   ...
   *
   * <p>This convenience method constructs a new {@link CommandOpMode}, adds a factory that returns
   * it to the underlying {@link OpModeRobot} registry, and associates it with {@link
   * RobotMode#AUTONOMOUS} along with the provided metadata.
   *
   ...
   * @return the created {@link CommandOpMode} instance for further setup
   */
  public CommandOpMode teleop(
      String name, String group, String description, Color textColor, Color backgroundColor) {
    var opMode = new CommandOpMode();
    addOpModeFactory(
        () -> opMode, RobotMode.TELEOPERATED, name, group, description, textColor, backgroundColor);
    return opMode;
  }
   ...
   * @return the created {@link CommandOpMode} instance for further setup
   */
  public CommandOpMode testOpMode(
      String name, String group, String description, Color textColor, Color backgroundColor) {
    var opMode = new CommandOpMode();
    addOpModeFactory(
        () -> opMode, RobotMode.TEST, name, group, description, textColor, backgroundColor);
    return opMode;
  }
