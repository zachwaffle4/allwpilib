...
public class InternalButton extends Trigger {
  ...
  /** Creates an InternalButton that is not inverted. Uses the default scheduler and universal context. */
  public InternalButton() {
    this(false);
  }

  /**
   * Creates an InternalButton which is inverted depending on the input. Uses the default scheduler and universal context.
   *
   * @param inverted if false, then this button is pressed when set to true, otherwise it is pressed
   *     when set to false.
   */
  ...
  /**
   * Creates an InternalButton with the given context and inversion setting. Uses the default scheduler.
   *
   * @param context The context that must be true for edges to be considered
   * @param inverted if false, then this button is pressed when set to true, otherwise it is pressed
   *     when set to false.
   ...
   */
  private InternalButton(Scheduler scheduler, Context context, AtomicBoolean state, AtomicBoolean inverted) {
